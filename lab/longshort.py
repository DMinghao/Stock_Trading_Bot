...
def getTradeable(conn): 
  ...
  return [x['symbol'] for x in finalList]

class LongShort:
  def __init__(self):
    self.alpaca = tradeapi.REST(key_id=API_KEY, secret_key=API_SECRET, base_url=APCA_API_BASE_URL, api_version='v2')
  ...
  def run(self):
    ...
    while True:
      ...
      if(self.timeToClose < (60 * 15)):
        ...
        #   tSubmitOrder.name = str(qty+ position.symbol+ orderSide+ respSO)
          tSubmitOrder.start()
          tSubmitOrder.join()
  ...
  def rebalance(self):
    ...
    for position in positions:
      if(self.long.count(position.symbol) == 0):
        ...
        #   tSO.name = str( position.symbol+ side+ respSO)
          tSO.start()
          tSO.join()
        else:
        ...
        #   tSO.name = str( position.symbol+ "buy"+ respSO)
          tSO.start()
          tSO.join()
        else:
    ...
    if(self.adjustedQLong > -1):
      ...
      for stock in respSendBOLong[0][0]:
        ...
        # tResendBOLong.name = 'tResendBOLong'
        tResendBOLong.start()
        tResendBOLong.join()
    ...
    if(self.adjustedQShort > -1):
      ...
      for stock in respSendBOShort[0][0]:
        ...
        # tResendBOShort.name = 'tResendBOShort'
  ...
  def sendBatchOrder(self, qty, stocks, side, resp):
    ...
    for stock in stocks:
      if(self.blacklist.isdisjoint({stock})):
        ...
        # tSubmitOrder.name = 'tSubmitOrder'
        tSubmitOrder.start()
        tSubmitOrder.join()
        if(not respSO[0]):
